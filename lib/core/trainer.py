...
from lib.utils.utils2 import norm_search
scaler = GradScaler() # half precision
logger = logging.getLogger(__name__)
# from thop import profile
H36M_14 = [6, 5, 4, 1, 2, 3, 13, 12, 11, 14, 15, 16, 8, 10]
H36M_14 = [3*i+j for i in H36M_14 for j in range(3)]
class Trainer():  # merge

    def __init__(self,
                 cfg,
                 train_dataloader:RandomIter,
                 test_dataloader,
                 model,
                 loss,
                 writer,
    ...
    def run(self):
        logger.info("\n")
        for di in [0]:
            self.test_dataloader[0].collate_fn.__self__.index = di
            performance = self.evaluate(flg=2, check=2)
            # os._exit(0)
        self.save_epoch = self.end_epoch // 7
        for epoch_num in range(self.start_epoch, self.end_epoch):
            self.epoch = epoch_num
            self.train()
            if  (self.epoch == self.end_epoch - 1) or \
                (self.epoch % (self.save_epoch + 1) == self.save_epoch):
                # (self.epoch % 6 == 5):
                # performance = None
                # for i in [0,1,2]:
                for i in [0]:
                    self.test_dataloader[0].collate_fn.__self__.index = i
                    performance = self.evaluate(2)
                    with open(self.cfg.LOGDIR+'/val.txt', 'a') as f:
                        print(self.epoch, file=f)
                        print(f'{i}th search', file=f)
    ...
    def train(self):
        ...
        bar = Bar(f'Epoch {self.epoch + 1}/{self.end_epoch}',
                  fill='*',
                  max=train_dataloader.dataset.__len__())
        for data in tqdm(train_dataloader, total=len(train_dataloader)) :

            data_pred = data["pred"].to(self.device) # data['gt'] [1024, 4, 8, 51] batch,person,frame,3dcoord
            data_search = data["search"].to(self.device) # data['gt'] [1024, 4, 8, 51] batch,person,frame,3dcoord
            data_gt = data["gt"].to(self.device) # data['pred'] [1024, 4, 8, 51]
            data_distance = data["distance"].to(self.device) # [1024, 4]


            
            data_search = norm_search(data_pred, data_search, coor_dim=train_dataloader.dataset.coor_dim) # norm data_search
            ...
            start = time.time()
            with autocast():
                denoised_pos = self.model(data_pred, data_search, data_distance)  # [1024, 4, 8, 51] # flops 227838787584.0 params 5385907.0
            ...
            comment = open(self.cfg.LOGDIR+'/comment.txt', 'r').read().replace('\n', '')
            summary_string = f'(Comment {comment} Iter {bar.index} | Total: {bar.elapsed_td} | ' \
    ...
    def evaluate(self, flg, check=-1):
        [i.dataset.set_check(check) for i in self.test_dataloader]
        self.model.eval()

        performance=[]
        all_dataset=self.cfg.DATASET_NAME
        ...
        for dataset_index in range(1):
            present_representation= self.test_dataloader[dataset_index].dataset.return_type
            present_dataset=self.test_dataloader[dataset_index].dataset.dataset_name
            present_estimator=self.test_dataloader[dataset_index].dataset.estimator
            print("=======================================================")
