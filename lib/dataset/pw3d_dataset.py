...
FLATTEN_H36M_TO_J14 = [3*a+b for a in np.array(H36M_TO_J17)[J17_TO_J14] for b in range(3)]
class PW3DDataset(BaseDataset):
    ...
    def get_data(self, index, norm=False): # shm
        ...
        if self.slide_window_size <= ground_truth_data_len:
            ...
            gt_data = deepcopy(gt_data[start_idx:end_idx, :])[:, FLATTEN_H36M_TO_J14]
            pred_data = deepcopy(pred_data[start_idx:end_idx, :])[:, FLATTEN_H36M_TO_J14]
            # if self.cflag:
                # self.cflag = False
        else:
            raise NotImplementedError
