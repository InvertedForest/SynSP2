...
from lib.utils.utils2 import norm_search, batch_compute_similarity_transform_torch
...
def evaluate_smoothnet_2D(model,
    ...
    if dataset =="h36m":
        ...
        for i, data in tqdm(enumerate(test_dataloader), total=test_dataloader.dataset.__len__()):
            ...
            data_search = data["search"].to(device)
            data_distance = data["distance"].to(device) # [1024, 4]

            with torch.no_grad():
                # data_pred=data_pred.permute(0,2,1)
                denoised_pos_a = model(
                    data_pred, data_search, data_distance)
                denoised_pos = denoised_pos_a[flg]
                    # data_pred=data_pred.permute(0,2,1)
    ...
    # evaluate on jhmdb
        ...
        for i, data in tqdm(enumerate(test_dataloader), total=test_dataloader.dataset.__len__()):
            ...
            with torch.no_grad():
                ...
                denoised_pos = model(data_pred)[flg]
...
def evaluate_smoothnet_3D(model, test_dataloader, device, dataset_name,estimator,cfg,flg):
    ...
    for i, data in tqdm(enumerate(test_dataloader), total=test_dataloader.dataset.__len__()):
        data_pred = data["pred"].to(device)
        data_gt = data["gt"].to(device)
        data_search = data["search"].to(device)
        data_distance = data["distance"].to(device) # [1024, 4]
        
        # random_index = 0
        # data_search = data_search[:,random_index]
        # data_distance = data_distance[:,random_index]
        data_search = norm_search(data_pred, data_search, coor_dim=test_dataloader.dataset.coor_dim)
...
# #########
#             start_time=time.time()
#             model(data_pred[:1416], data_search[:1416], data_distance[:1416])
#             print(1)
...
# #######
        ...
        # denoised_pos = denoised_pos[:, :, H36M_14]
...
def evaluate_smoothnet_smpl(model, test_dataloader, device,cfg,dataset, flg):
    ...
    for i, data in tqdm(enumerate(test_dataloader), total=test_dataloader.dataset.__len__()):
        ...
        data_search = data["search"].to(device)
        data_distance = data["distance"].to(device) # [1024, 4]
        data_pred_shape = data["shape"].to(device).squeeze(0)
        data_gt_trans = data["trans"].to(device).squeeze(0)
        data_gt_scaling = data["scale"].to(device).squeeze(0)
        if len(data_pred.shape) != 3: continue
